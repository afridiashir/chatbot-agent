...
/**
 * The emoji panel for the agent's composer: category tabs with an underline
 * that follows the scroll, a search box, a Recent row and the grid. Emoji
 * buttons don't take focus on mouse down, so the caret stays in the message
 * box. Mirrors the widget's picker, so both sides behave alike.
 */
export function EmojiPicker({
  ...
  return (
    ...
    >
      <div className="flex border-b border-border" role="tablist" aria-label="Emoji categories">
          ...
          return (
            <button
              ...
              className={`relative flex h-10 flex-1 items-center justify-center transition ${
                selected ? "text-primary" : "text-chat-meta hover:text-foreground"
              }`}
            >
              <svg
                viewBox="0 0 24 24"
              ...
              <span
                className={`absolute inset-x-1.5 bottom-0 h-[3px] rounded-t-full bg-primary transition-opacity ${
      ...
      >
        {results ? (
          results.length > 0 ? (
            ...
            <p className="py-8 text-center text-sm text-chat-meta">No emoji found</p>
          )
        ) : (
          sections.map((section) => (
            ...
            >
              <h3 className="px-1 pt-2.5 pb-1 text-[13px] text-chat-meta">{section.label}</h3>
              {grid(section.emoji)}
            </section>
          ))
        )}
